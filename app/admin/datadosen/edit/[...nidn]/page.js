...
const formSchema = z.object({
  nidn: z.string().min(5, "NIDN Minimal 5 Digit").max(20, "NIDN Maksimal 20 Digit").regex(/^\d+$/, "NIDN Harus Angka"),
...
const EditDataDosen = () => {
  ...
  // Extract the nidn parameter from the URL path
  const nidn = pathname.split('/').pop();
  ...
  useEffect(() => {
    if (nidn) {
      const fetchData = async () => {
        setLoading(true);
        try {
          const response = await fetch(`/api/dosen/edit?nidn=${nidn}`);
          if (!response.ok) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        Swal.fire({
          title: "Berhasil",
          text: "Data yang di edit berhasil disimpan",
          icon: "success",
        });
